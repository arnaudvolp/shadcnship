...
import { getSaasBlock, getSaasBlocks } from "@/lib/registry";
...
export async function generateMetadata({
  ...
  const block = getSaasBlock(name);

  if (!block) {
    return constructMetadata({
      title: "SaaS Block Not Found",
      description: "The requested SaaS block could not be found.",
      noIndex: true,
    });
  }

  const primaryCategory = block.categories[0];
  const categoryTitle = primaryCategory?.title || "SaaS";

  // Generate block-specific keywords
  const keywords = generateBlockKeywords(block.title, categoryTitle);

  return constructMetadata({
    title: `${block.title} - ${categoryTitle} SaaS Block`,
    ...
    keywords,
    alternates: {
      canonical: absoluteUrl(`/saas/${block.name}`),
    },
    openGraph: {
      type: "article",
    },
...
export default async function SaasBlockPage({
  params,
}: {
  params: Promise<{ name: string }>;
}) {
  ...
  const blockJsonLdData = {
    ...
    category: primaryCategory?.title || "SaaS",
    categorySlug: primaryCategory?.name || "saas",
    image: block.image,
  };
